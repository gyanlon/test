...
public class WoSaleOrderInBirdStage extends TaskStage {

	private static final Logger LOGGER = LoggerFactory.getLogger(WoSaleOrderInBirdStage.class);
	...
	boolean internal(ApplicationContext appContext, TaskContext taskContext) {
		...
		for (NlbpSysOrderProcessModel orderProcessModel : recordsSale) {
			...
			NlbpWsSaleOutputBean outBean = workOrderService.inSaleOrder(beanList);
	...
	WorkOrderInterfaceType getInterfaceType() {
		return WorkOrderInterfaceType.WORK_ORDER_SALE_IN;
	}
}