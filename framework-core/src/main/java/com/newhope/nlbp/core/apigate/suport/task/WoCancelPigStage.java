 ...
 * @since 2017-07-10
 */
public class WoCancelPigStage extends TaskStage {

	private static final Logger LOGGER = LoggerFactory.getLogger(WoCancelPigStage.class);

	@Override
	boolean internal(ApplicationContext appContext, TaskContext taskContext) {
		List<NlbpSysOrderProcessModel> recordsCP = getWaitDealRecordList(taskContext);

		if (CollectionUtils.isEmpty(recordsCP)) {
			return true;
		}
		WorkOrderProcessService workOrderService = appContext.getBean(WorkOrderProcessService.class);
		boolean b = true;
		// 调用接口及更新状态
		for (NlbpSysOrderProcessModel orderProcessModel :  recordsCP) {
			...
			NlbpWsChangeWorkOrderOutputBean outBean = workOrderService.CancleWorkOrderPig(inputBean);
	...
	WorkOrderInterfaceType getInterfaceType() {
		return WorkOrderInterfaceType.WORK_ORDER_CANCEL_PIG;
	}
}
