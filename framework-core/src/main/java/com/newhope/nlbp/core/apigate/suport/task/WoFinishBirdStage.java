 ...
 * @since 2017-07-10
 */
public class WoFinishBirdStage extends TaskStage {

	private static final Logger LOGGER = LoggerFactory.getLogger(WoFinishBirdStage.class);

	@Override
	boolean internal(ApplicationContext appContext, TaskContext taskContext) {
		List<NlbpSysOrderProcessModel> recordsFB = getWaitDealRecordList(taskContext);
		if (CollectionUtils.isEmpty(recordsFB)) {
			return true;
		}
		WorkOrderProcessService workOrderService = appContext.getBean(WorkOrderProcessService.class);
		boolean b = true;
		// 调用接口及更新状态
		for (NlbpSysOrderProcessModel orderProcessModel : recordsFB) {
	...
	WorkOrderInterfaceType getInterfaceType() {
		return WorkOrderInterfaceType.WORK_ORDER_FINISH_BIRD;
	}
}
